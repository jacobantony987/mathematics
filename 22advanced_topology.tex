...
\begin{proof}
	...
	Consider \textcolor{blue}{$E = \{ (n,U) \in D \times \mathcal{N}(x) : S(n) \in U \}$} and define $\ge'$ by $(n,U) \ge' (m,V)$ if $n \ge m$ and $U \subset V$. Trivially, $(n,U) \ge' (m,V) \ge' (p,W) \implies (n,U) \ge' (p,W)$ and $(n,U) \ge' (n,U)$. Also, for any $(n,U),(m,V) \in E$, we have $n,m \in D$ and $U,V \in \mathcal{N}(x)$. Since $D$ is a directed set, $\exists p \in D$ such that $p \ge n$ and $p \ge m$. \textcolor{blue}{Also, there exists $q \in D$ such that $S(q) \in U$ and $q \ge' p$, since $S$ is frequently in every nbd $U$ of $x$}. And $U \cap V \in \mathcal{N}(x)$ such that $U \cap V \subset U$ and $U \cap V \subset V$. Thus $\exists (q,U\cap V) \in E$ such that $(q,U\cap V) \ge' (n,U)$ and $(q,U\cap V) \ge' (m,V)$. Therefore, $(E,\ge')$ is a directed set.\\

	Define $N : E \to D$ by $N(n,U) = n$. Again for any $(m,V) \in E$, there exists $m \in D$ such that $(n,U) \ge' (m,U)$ implies there exists $n \in D$ such that  $N(n,U) = n$ and $n \ge m$. Now, we have $T : E \to X$ defined by $T(n,U) = S(N(n,U)) = S(n)$. Therefore, $T$ is a \textcolor{blue}{subnet} of $S$. \textcolor{blue}{Now every points in $E$ are so defined that $(n,U) \in E \implies S(n) \in U$, we have $T(n,U) \in U$. Thus, $T$ is eventually in every nbd of $x$. ie, subnet $T$ of net $S$ is convergent to $x$.}
\end{proof}

\begin{remark}
	\begin{itemize}
		\item  \textcolor{blue}{Importance of Construction of $E$}\\ If $x$ is a cluster point of a net $S$ in $X$, then $S$ is frequently in some cofinal subset of $D$. Thus, \textcolor{blue}{if we consider any cofinal subset $D'$ of $D$} which is a direct set with $\ge$ restricted to \textcolor{blue}{$D'$}. Then $N : D' \to D$ defined by $N(n)=n$ gives a \textcolor{blue}{ subnet $T : D' \to X$ of the net } $S$. However, this subnet need not converge to $x$. The strongest statement, we can make on $T$ is that `$x$ is a cluster point of $T$' \textcolor{blue}{, since $N : D \times \mathcal{N}(x) \to D,\ N(n) = n$ is completely independent of $U$. This problem is overcame by constructing $E$ which is dependent on each nbd $U$ of $x$.}
		\item \textcolor{blue}{Existence of $q \in D$ such that $q \ge' p$ and $S(q) \in U$. We have, $p$ follows both $n \& m$ and $U \cap V$ is a subset of both $U \& V$. However, since $S$ is only frequently in $U$, $p$ not necessarily be in $U$. But there is always someone following $p$ which has its image in $U$. This $q$ follows both $n \& m$, since $\ge'$ is transitive.}
	\end{itemize}
\end{remark}
...
\begin{definition}
	Let $X,\ Y$ be topological spaces and $f : X \to Y$, $f':X \to Y$ be continuous functions. Then $f,\ f'$ are homotopic if there exists a continuous function $F : X \times I \to Y$ such that for every $x \in X$, $F(x,0) = f(x)$ and $F(x,1) = f'(x)$. And we write, $f \cong f'$.
\end{definition}

\begin{definition}
	Let $Y$ be a topological space and $f : I \to Y$ and $f' : I \to Y$ be two paths. Then $f,\ f'$ are path-homotopic if they have same initial point $x_0$ (ie, $x_0=f(0)=f'(0)$), same final point $x_1$ (ie, $x_1=f(1)=f'(1)$) and they are homotopic (ie, $\exists F: I \times I \to Y$ such that $\forall x \in I$, $F(x,0)=f(x)$ and $F(x,1)=f'(x)$ also fixed at the end points $x_0$ and $x_1$(ie, $\forall t \in I$, $F(0,t) = x_0$ and $F(1,t) = x_1$). And we write $f \cong_p f'$.
\end{definition}
...
\begin{proposition}
	The relations $\cong,\ \cong_p$ are equivalence relations.
\end{proposition}

\begin{remark}[Straight line homotopy]
	Let $X$ be a topological space, and $f,\ g$ be continous functions from $X$ into a eucilidean space, say $\mathbb{R}^2$. Then $f,\ g$ are straight line homotopic if there exists a continuous function $F$ from $X \times I$ such that $F$ deforms $f$ into $g$ along straight line segments joining them.\\
