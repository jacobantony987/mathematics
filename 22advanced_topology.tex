...
\begin{proof}
	...
	We have, $T$ is a subnet of $S$. Then $\exists N : E \to D$ such that\\
	$\forall m' \in D,\ \exists p \in E$ such that $n \ge' p \implies N(n) \ge m'$.\\
	And $p \ge' p \implies N(p) = p' \ge m'$ and $S(p') = S(N(p)) = T(p) \in T(E)$.\\
	Therefore, $\forall m' \in D$, $\exists p' \in D$ such that $S(p') \in T(E)$.\\
	Thus $S$ is frequently in $T(E)$.\\

	We have, $T$ converges to $x$. ie, $T$ is eventually in each nbd $U$ of $x$. Therefore, $S$ is frequently in each nbd $U$ of $x$. ie, $x$ is a cluster point of $S$.\\

	We have, $T$ converges to $x$.\\
	Thus for each nbd $U$ of $x$, $\exists m \in E$ such that $n \ge' m \implies T(n) \in U$.\\
	Also, $m' \in D \implies \exists p \in E$ such that $n \ge' p \implies N(n) \ge m'$.\\
	We have, $m,p \in E \implies \exists q \in E$ such that $q \ge' m$ and $q \ge' p$.
\end{proof}
