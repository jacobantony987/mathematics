...
\begin{description}
	...
	\item[Cartesian Coordinate Function] Let $U \subset \mathbb{R}^{n+1}$. Then $i$th cartesian coordinate function $x_i$ is given by $x_i : U \to \mathbb{R},\ x_i(a_1,a_2,\cdots,a_{n+1}) = a_i$.
	\item[$dx_i$] The differential of the cartesian coordinate function $x_i$ is given by $dx_i$ where $dx_i : U \times R^{n+1} \to \mathbb{R}$
		$$dx_i = \nabla x_i(p) \cdot \mathbf{v} = (p,0,0,\cdots,1,\cdots,0) \cdot (p,v_1,v_2,\cdots,v_{n+1}) = v_i$$
\end{description}
\subsection{$1$-form}
\begin{description}
...
\end{proposition}
\begin{proof}
	Let $\mathbf{X}_j$ be a smooth vector field on $U$ where $X_j(p) = (p,0,0,\cdots,1,\cdots,0)$. Then, $dx_i(X_j) =\delta_{ij}$. Suppose $\omega = \sum f_i dx_i$. Then 
...
\begin{description}
	...
	In our example, $S \cap \mathcal{N}(\mathbf{v})$ is the skin of cross-section of that apple.
\end{description}
...
\end{theorem}
\begin{proof}
	Let $f : U \to \mathbb{R}$ such that $S = f^{-1}(c)$ and $\nabla f(q) \ne \mathbf{0},\ \forall q \in S$. Let $\nabla f(q) = \left(q,\widetilde{\nabla f}(q)\right)$. Let $p \in S$ and $\mathbf{v} \in S_p$. Let $i : \mathbb{R}^2 \to \mathbb{R}^{n+1}$ defined by $i(x,y)=p+xv+yN(p)$. Clearly, the image of the function $i$ is the normal section, $\mathcal{N}(\mathbf{v}) = i(\mathbb{R}^2)$.\\
...
\end{lemma}
\begin{description}
	\item[self-adjoint] That is, $L(v)\cdot w = v \cdot L(w)$.
	\item[$S = \{ v \in V : v \cdot v = 1\}$] That is, $S$ is the set of all unit vectors in $V$
	\item[$f$ is stationary at $v_0$] That is, for every parametrised curve $\alpha : I \to S$ with $\alpha(t_0) = v_0$,  $(f \circ \alpha)'(t) = 0$
...
\begin{proof}
	...
	\begin{align*}
		...
		= & \dfrac{d}{dt} \left( \cos^2 t\ L(v_0) \cdot v_0 + 2\sin t \cos t\ L(v_0) \cdot v + \sin^2 t\ L(v) \cdot v \right)(0)\\
...
\end{theorem}
\begin{proof}--- yet to complete --- 4
\end{proof}

\begin{theorem}
	Let $S$ be an oriented $n$-surface in $\mathbb{R}^{n+1}$. Let $p in S$ and $\{ \kappa_1(p),\kappa_2(p),\cdots,\kappa_n(p) \}$ be the principal curvatures of $S$ at $p$ with corresponding orthogonal principal curvature directions $\{ \mathbf{v}_1,\mathbf{v}_2, \cdots, \mathbf{v}_n \}$. Then the normal curvature $\kappa(\mathbf{v})$ in the direction $\mathbf{v} \in S_p$ is given by
	\begin{equation}
...
\end{theorem}
\begin{proof}--- yet to complete --- 6
\end{proof}

\begin{theorem}
	Let $S$ be an $n$-surface in $\mathbb{R}^{n+1}$. Let $p \in S$. Let $\mathbf{Z}$ be any  non-zero normal vector field on $S$ such that $\mathbf{N} = \mathbf{Z}/\|\mathbf{Z}\|$ and let $\{\mathbf{v}_1,\mathbf{v}_2,\cdots,\mathbf{v}_n\}$ be any basis for $S_p$. Then
	\begin{equation}
...
\begin{description}
	...
	\begin{equation}
		\tilde{\varphi}(u_1,u_2,\cdots,u_{n+1}) = (\varphi(u_1,u_2,\cdots,u_n),u_{n+1})
	\end{equation}
...
\begin{definition}
	...
	\begin{equation}
		\mathbf{E}_i(p) = d\varphi_p(p,0,0,\cdots,1,\cdots,0)
	\end{equation}
\end{definition}

\begin{theorem}[Basis for Tangent Space Image $d\varphi_p$]
	Let $\varphi : U \to \mathbb{R}^{n+k}$ be smooth. Then the set of all coordinate vector fields $\{ \mathbf{E}_i(p) : i = 1,2,\cdots,n \}$ form a basis for the tangent space image $d\varphi_p$.
\end{theorem}
