...
\begin{description}
	...
	\end{equation}
	\textbf{Note} : Length of arc is the total distance travelled. For example : $\alpha_1 : [0,2\pi] \to \mathbb{R}^2$ defined by $\alpha_1(t) = (\cos t,\sin t)$ has its length $2\pi$, the perimeter of unit circle. But, for $\alpha_2 : [0,4\pi] \to \mathbb{R}^2$ defined by $\alpha_2(t) = (\cos t,\sin t)$ has its length $4\pi$. And $\alpha : \mathbb{R} \to \mathbb{R}^2$ defined by $\alpha(t) = (\cos t,\sin t)$ has its length $+\infty$.
\end{description}
...
\begin{proof}
	...
	{Necessary Part : } Suppose the plane curve $C$ is connected. Let $p \in C$ and $\alpha$ be the local parametrisation of $C$ at $p$, which is nothing but the maximal integral curve through $p$ in the tangent space obtained by rotating $\nabla f$ by an angle of $-\pi/2$ where $C = f^{-1}(c)$ and orientation $\mathbf{N} = \nabla f / \| \nabla f \|$. --- yet to complete ---
...
\begin{description}
	...
	\item[1-form] A function $\omega : U \times R^{n+1} \to \mathbb{R}$  where $U \subset \mathbb{R}^{n+1}$ and $p \in U \implies \mathbb{R}_p^{n+1} \subset U \times \mathbb{R}^{n+1}$. Examples, $\omega_\mathbf{X}$, $df$ and $dx_i$. (below)\\
