...
\begin{proof}
	to be updated\\
	To prove that $P_\alpha$ is a vector space isomorphism preserving dot products, we need to prove that following
\begin{enumerate}
	\item $P_\alpha$ is linear, since vector space isomorphism preserves linearity\\
		$P_\alpha(\mathbf{v}+\mathbf{w}) = P_\alpha(\mathbf{v}) + P_\alpha(\mathbf{w})$ and $P_\alpha(c\mathbf{v}) = cP_\alpha(\mathbf{v})$
	\item $P_\alpha$ is bijective, since isomorphism are bijective
