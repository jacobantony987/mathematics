...
\begin{proof}
	...
	$$\displaystyle \hat{d}(\hat{a},\hat{b}) = \hat{d}(Ta,Tb) = \lim_{k \to \infty} d(a,b) = d(a,b)$$
...
\begin{proof}
	...
	%Let $A : W \to X$ be the bijective, isometry suggested by the completion theorem of metric spaces.
	%Let $\hat{x} \in W$ and $A(\hat{x}) = x \in X$.
	%Define $\| \cdot \| : W \to \mathbb{R}$ defined by $\| \hat{x} \| = \| x \|$.
	...
	\begin{enumerate}
		\item 
	%		We have $\| \cdot \|$ is continuous on $\hat{X}$ and $\forall \hat{x} \in W,\ \| \hat{x} \| = \| x \|$.
	%		$\forall \hat{y} \in \hat{X}$, as $\hat{x} \to \hat{y}$, we have $\| \hat{x} \| \to \| \hat{y} \|$
	\end{enumerate}
\end{proof}
