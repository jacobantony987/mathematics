...
\begin{proof}
	...
	Then,
	\[ \forall \varepsilon > 0,\ \exists N \in \mathbb{N} \text{ such that } \forall n > N,\ \| f_n-f_N \| < \varepsilon \]
	We have continous functions on compact sets are uniformly continuous.
	Thus there exists a natrual number $N$ such that for every natural numbers $n,m$ greater than $N$, we have $|f_n(x) - f_m(x)| < \varepsilon$.
	Let $x \in K$.
	Then,
	\[ |f_n(x)-f_m(x)| \le |f_n(x)-f_N(x)| + |f_N(x) - f_N(y)| + |f_N(y) - f_n(y)| < 3\varepsilon \]
	Since $x$ is arbitrary, we have $\sequence{f_n}$ satisfied the inequailty.
	Therefore, $\sequence{f_n}$ is equicontinuous on $K$.
\end{proof}
