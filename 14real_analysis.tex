...
\begin{proof}
	...
	\begin{align*}
		...
		& = f(t) Q_n^{(1)}(t-x) - \int_0^1 f(t) Q_n^{(1)}(t-x)\ dt \text{ by product rule}\\
		& = f(t)\ Q_n^{(1)}(t-x) -f(t)\ Q_n^{(2)}(t-x) + \dots - (-1)^m\ f(t)\ Q_n^{(m)}(t-x)
	\end{align*}
	Thus, $\sequence{P_n}$ is a sequence of polynomials.
