...
\item \textbf{Monotone function on an interval is measurable}\\
	\begin{proof}
		...
		Thus, $f$ is defined on a measurable set.
		Without loss of generality, suppose that $f(x)$ is an increasing function.
		Then $\{ g_n \}_{n=1}^\infty$ where $g_n(x) = f(x) + \frac{x}{n}$ is a collection of strictly increasing functions.
		---yet to prove that $g_n$ are measurable.
		---then $\lim_{n \to \infty} g_n = f$ is also measurable
	\end{proof}
...
\item \textbf{Composition of measurable functions is not necessarily measurable}\\
	We know that, Cantor set $C$ has subset $A$ such that $\psi$ maps $A$ onto a non-measurable set.
	Let $A$ be a measurable subset of open interval $(0,1)$ such that $\psi(A)$ is non-measurable.\\
		
	Let $\chi_A$ be the characteristic function of $A$.
	Then $\chi_A \circ \psi^{-1}$ is not measurable.
	But, both the functions $\chi_A$ and $\psi^{-1}$ are measurable functions.
