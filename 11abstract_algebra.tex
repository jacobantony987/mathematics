...
\begin{definition}[Cartesian product of sets]
	Let $S_1,\ S_2, \dots,\ S_n$ be a sets.
	Their cartesian product,
	\begin{equation}
		S_1 \times S_2 \times \dotsb \times S_n = \prod_{i = 1}^n S_n = \{ (a_1,a_2,\dots,a_n) :  a_i \in S_i \} 
	\end{equation}
...
\begin{theorem}[Direct product of Groups]
	...
	\begin{equation}
		(a_1,\ a_2,\ \dots,\ a_n) \ast (b_1,\ b_2,\ \dots,\ b_n) = (a_1 \ast b_1,\ a_2 \ast b_2,\ \dots,\ a_n \ast b_n)
	\end{equation}
...
\begin{proof}
	...
	\begin{enumerate}[label=G\arabic*]
		\item Associativity
			\begin{align*}
				(a_1,\ a_2,\ & \dots,\ a_n) \big( (b_1,\ b_2,\ \dots,\ b_n)(c_1,\ c_2,\ \dots,\ c_n) \big) \\
				= & (a_1,\ a_2,\ \dots,\ a_n)(b_1c_1,\ b_2c_2,\ \dots,\ b_nc_n) \\
				= & \big( a_1(b_1c_1),\ a_2(b_2c_2),\ \dots,\ a_n(b_nc_n) \big) \\
				= & \big( (a_1 b_1)c_1,\ (a_2b_2)c_2,\ \dots,\ (a_nb_n )c_n \big) \\
				= &  (a_1b_1,\ a_2b_2,\ \dots,\ a_nb_n)(c_1,\ c_2,\ \dots,\ c_n) \\
				= & \big( (a_1,\ a_2,\ \dots,\ a_n)(b_1,\ b_2,\ \dots,\ b_n) \big)(c_1,\ c_2,\ \dots,\ c_n)
			\end{align*}
		...
		\item Existence of a unique identity element in $\prod\limits_{i = 1}^n G_i$\\
			...
			Then $(e_1,\ e_2,\ \dots,\ e_n)$ is the identity element in $\prod\limits_{i = 1}^n G_i$.
			\begin{align*}
		...
		\item Existence of unique inverse element for each element in $\prod\limits_{i = 1}^n G_i$\\
			Let $(a_1,\ a_2,\ \dots, a_n)$ be in $\prod\limits_{i = 1}^n G_i$.
			Then it has the inverse element $( a_1^{-1},\ a_2^{-1},\ \dots,\ a_n^{-1} )$ in $\prod\limits_{i = 1}^n G_i$.
			\begin{align*}
				(a_1,\ a_2,\ \dots a_n)&(a_1^{-1},\ a_2^{-1},\ \dots, a_n^{-1}) \\
				& = (a_1a_1^{-1},\ a_2a_2^{-1},\ \dots,\ a_na_n^{-1}) \\
				& = (e_1,\ e_2,\ \dots,\ e_n)
			\end{align*}
	\end{enumerate}
\end{proof}
...
\begin{remark}
	$\mathbb{Z}_n = \{ 0,1,\dots,(n-1) \}$ is a group with $+_n$.
	(addition modulo $n$)
...
\begin{corollary}
	$\prod\limits_{i = 1}^n \mathbb{Z}_{m_i}$ is cyclic and is isomorphic to $Z_{m_1m_2\dots m_n}$ if and only if any two of the numbers $m_i$ are relatively prime.
\end{corollary}
...
\begin{theorem}
	Let $(a_1,\ a_2,\ \dots,\ a_n) \in \prod\limits_{i = 1}^n G_i$. And $a_i$ are of finite order $r_i$ in $G_i$. Then the order of $\prod\limits_{i = 1}^n G_i$ is the least common multiple of $r_i$s.
\end{theorem}
\begin{proof}
	...
	Thus, the order of the cyclic subgroup generated by $(a_1,\ a_2,\ \dots,\ a_n)$ is the least common multiple of all the $r_i$s.
\end{proof}
\begin{remark}
...
\begin{remark}
	Define $\overline{G_i} = \{ (e_1,\ e_2,\ \dots,\ e_{i-1},\ a_i,\ e_{i+1},\ \dots,\ e_n) : a_i \in G_i\}$.
...
\begin{theorem}[fundamental theorem of finitely generated abelian groups]
	...
	\begin{equation}
		\mathbb{Z}_{{p_1}^{r_1}} \times\mathbb{Z}_{{p_2}^{r_2}} \times \dotsb \times \mathbb{Z}_{{p_n}^{r_n}} \times \mathbb{Z} \times \mathbb{Z} \times \dotsb \times \mathbb{Z}
	\end{equation}
...
\begin{proof}
	...
	\[ G \approx \mathbb{Z}_{{p_1}^{r_1}} \times \mathbb{Z}_{{p_2}^{r_2}} \times \dotsb \times \mathbb{Z}_{{p_n}^{r_n}}\]
...
\end{theorem}
\begin{proof}
	Let $G \approx \mathbb{Z}_{{p_1}^{r_1}} \times \mathbb{Z}_{{p_2}^{r_2}} \times \dotsb \times \mathbb{Z}_{{p_n}^{r_n}}$.
	Then $|G| = p_1^{r_1} p_2^{r_2} \dotsm p_n^{r_n}$.
	Suppose $m$ divides $|G|$, then $m = p_1^{s_1} p_2^{s_2} \dotsm p_n^{s_n}$ where $0 \le s_i \le r_i$.
	Define $H = \mathbb{Z}_{{p_1}^{s_1}} \times \mathbb{Z}_{{p_2}^{s_2}} \times \dotsb \times \mathbb{Z}_{{p_n}^{s_n}}$.
...
\begin{proof}
	...
	Therefore, $G \approx \mathbb{Z}_{p_1 p_2 \dotsm p_n}$ is a cyclic group of order $m$.
\end{proof}
%\pagebreak
...
\begin{definition}
	The integers $m_1,m_2,\dots,m_n$ are torsion coefficients of $G$ such that $G \approx \mathbb{Z}_{m_1} \times \mathbb{Z}_{m_2} \times \dotsb \times \mathbb{Z}_{m_n}$ where $m_i$ divides $m_{i+1}$.%\S11Ex44
\end{definition}
...
\begin{definition}
	A \textbf{permutation group} $S_n$ is the set of all permutations on the set $\{1,2,\dots,n\}$.	
\end{definition}
...
\begin{proof}
	\begin{figure}
		...
		\begin{tikzpicture}
			...
			\draw[dotted] (8,0) --node[above]{{\color{red}$\mu_2^{-1} \circ \mu_1$}} (4,0);
			\draw (1,2) node{$HN$};
...
\begin{proof}
	...
	Choose an element from each orbit, say $x_1,x_2,\dots,x_r$.
	We have, 
	\begin{equation}
		|X| = \sum_{i = 1}^r |Gx_i|
	\end{equation}
...
\begin{proof}
	...
	\begin{equation}
	X = \{ (g_1,g_2,\dots,g_p) \in G^p : g_1g_2 \dotsm g_p = e\}
	\end{equation}
	Let $g_1,g_2,\dots,g_{p-1}$ be any elements in $G$.
	Then $g_p = (g_1g_2 \dotsm g_{p-1})^{-1}$ is uniquely determined.
	That is, the $(p-1)$ co-ordinates of an element in $X$ may be chosen in $|G|$ different ways.
	Thus, $|X| = |G|^{p-1}$.
	Since $p$ divides $|G|$, $p$ divides $|X|$.

	We have $g_1(g_2 \dotsm g_p) = (g_2g_3 \dotsm g_p)g_1$, since $g_2g_3 \dotsm g_p = g_1^{-1}$.
	Thus, for every $(g_1,g_2,\dots,g_p) \in X$, the $\sigma$ permutation of that element is in $X$.
	That is, $\sigma(g_1,g_2,\dots,g_p) = (g_2,g_3,\dots,g_p,g_1) \in X$.
	Similarly, $g_2(g_3g_4 \dotsm g_pg_1) = (g_3g_4 \dotsm g_pg_1)g_2$.
	And $\sigma^2(g_1,g_2,\dots,g_p) = g_3,g_4,\dots,g_p,g_1,g_2)$.
	...
	Clearly, $(e,e,\dots,e) \in X$ and $(e,e,\dots,e) \in X_{\generator{\sigma}}$.
	Thus, $X_{\generator{\sigma}}$ has atleast $p$ elements.
	Let $(g_1,g_2,\dots,g_p) \in X_{\generator{\sigma}}$, then $\sigma$ fixes $(g_1,g_2,\dots,g_p)$.
	\begin{align*}
		\sigma(g_1,g_2,\dots,g_p) & = (g_1,g_2,\dots,g_p)\\
		\implies (g_2,\dots,g_p,g_1) & = (g_1,g_2,\dots,g_p)
	\end{align*}
	Thus, $g_1 = g_2 = \dotsb = g_p$, say $g \in G$.
	That $(g,g,\dots,g) \in X$ and $g^p = e$ by the definition of $X$.
...
%	That is, $H_0 \le H_1 \le \dots \le H_n = G$.
\end{theorem}
\begin{proof}
	...
	By mathematical induction, $G$ has subgroups of order $p^i$ for $i = 2,3,\dots,n$.
\end{proof}
...
\begin{definition}
	A group $G$ is solvable if there is sequence of subgroups $\{e\} = H_0 \le H_1 \le \dotsb H_n = G$ such that for $i = 0,1,2,\dotsc$
	\begin{enumerate}
...
\begin{remark}
Finite, non-abelian, simple groups are of order $60, 168, 360, \dotsc$. 
\end{remark}
